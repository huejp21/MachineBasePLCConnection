...
{
  public class ClassThread_Test2
  {
    private static ClassThread_Test2 cInstatnce;
    private static object syncLock = new object();

    private static System.Threading.Thread th = null;
    private static bool bThread = true;

    protected ClassThread_Test2()
    {
      Check_Thread();
    }

    public static ClassThread_Test2 Get_Instance()
    {
      if (cInstatnce == null)
      {
        lock (syncLock)
        {
          if (cInstatnce == null)
          {
            cInstatnce = new ClassThread_Test2();
          }
        }
      }
      return cInstatnce;
      ...
      while (bThread)
      {
        lock (this)
        {
          ...
          if (CVo.bTestMeasure2)
          {
            ...
            if (cSys.bUsePB3 == false && cSys.bUsePB4 == false)
            {
              continue;
            }
            ...
            for (int i = 0; i < CVo.iPointMax; i++)
            {
              ...
              if (cSys.bUsePB3 && cSys.bUsePB4 && cRcp.abUsePointX[i])
              {
                ...
                if (cRcp.abUsePointY_Two[i])
                {
                  ...
                  sData.ProbeInfo[iPointIndex1] = enumProbe.PR3;
                ...
                sData.PointInfo[iPointIndex2] = cRcp.abUsePointY_Two[i] ? enumPoint.Y2 : enumPoint.Y1;
                sData.ProbeInfo[iPointIndex2] = enumProbe.PR4;
              ...
              else if (cSys.bUsePB3 && cSys.bUsePB4 == false && cRcp.abUsePointX[i])
              {
                ...
                if (cRcp.abUsePointY_Two[i])
                {
                  ...
                  sData.OriginValue[iPointIndex2] = dValue2;
                  sData.X_Index[iPointIndex2] = i;
                  sData.ProbeInfo[iPointIndex2] = enumProbe.PR3;
              ...
              else if (cSys.bUsePB3 == false && cSys.bUsePB4 && cRcp.abUsePointX[i])
              {
                ...
                sData.X_Index[iPointIndex1] = i;
                sData.PointInfo[iPointIndex1] = enumPoint.Y1;
                sData.ProbeInfo[iPointIndex1] = enumProbe.PR4;
                sData.PointIndex.Add(iPointIndex1);
            ...
            {
              sData.ZeroValue[i] = CVo.asZeroData_Stage2[i].Value;
            }
            for (int i = 0; i < sData.PointIndex.Count; i++)
            {
              ...
              {
                case enumPoint.Y1:
                  dPointOffset = cRcp.adPointY1_Stage2_Offset[sData.X_Index[iIndex]];
                  break;
                case enumPoint.Y2:
                  dPointOffset = cRcp.adPointY2_Stage2_Offset[sData.X_Index[iIndex]];
                  break;
                case enumPoint.NONE:
                default:
                  break;
              ...
              if (true)
              {
                int x = sData.X_Index[iIndex];
                int y = sData.PointInfo[iIndex] == enumPoint.Y1 ? 0 : 1;
                iRangeIndex = x * 2 + y;
              }
              ...
              switch (eUEMode) // 측정 결과 초기화 및 결과 산출
              {
                ...
                case enumUEUseMode.USE_2:
                  ...
                  if (cRcp.abPointY_Range_Min_A[iRangeIndex] <= sData.MeasureValue[iIndex]
                   && cRcp.abPointY_Range_Max_A[iRangeIndex] >= sData.MeasureValue[iIndex])
                  {
                    sData.PointResult[iIndex] = enumPCBResult.A;
                  }
                  break;
                case enumUEUseMode.USE_3:
                  ...
                  else if (cRcp.abPointY_Range_Min_B[iRangeIndex] <= sData.MeasureValue[iIndex]
                        && cRcp.abPointY_Range_Max_B[iRangeIndex] >= sData.MeasureValue[iIndex])
                  {
                    sData.PointResult[iIndex] = enumPCBResult.B;
                  }
                  break;
                ...
                default:
                  ...
                  else if (cRcp.abPointY_Range_Min_C[iRangeIndex] <= sData.MeasureValue[iIndex]
                        && cRcp.abPointY_Range_Max_C[iRangeIndex] >= sData.MeasureValue[iIndex])
                  {
                    sData.PointResult[iIndex] = enumPCBResult.C;
                  }
                  break;
            ...
            try
            {
              string strFileFullPath = CVo.LOG_PATH_TEST + CVo.LOG_NAME_TEST_MEASURE2;
          ...
          if (CVo.bTestZeroSet2)
          {
            ...
            for (int i = 0; i < CVo.iPointMax; i++)
            {
              ...
              if (cSys.bUsePB3 && cSys.bUsePB4 && cRcp.abUsePointX[i])
              {
                ...
                if (cRcp.abUsePointY_Two[i])
                {
                  ...
                  sZeroData[iPointIndex1].ProbeType = enumProbe.PR3;
                }
                double dValue2 = CMaster.cDataMgr.Average_Calculate(list2.ToArray(), cSys.iProbeValueAverageMin, cSys.iProbeValueAverageMax);
                sZeroData[iPointIndex2].Value = dValue2;
                sZeroData[iPointIndex2].X = i;
                sZeroData[iPointIndex2].PointType = cRcp.abUsePointY_Two[i] ? enumPoint.Y2 : enumPoint.Y1;
                sZeroData[iPointIndex2].ProbeType = enumProbe.PR4;
              }
              else if (cSys.bUsePB3 && cSys.bUsePB4 == false && cRcp.abUsePointX[i])
              {
                // 프로브 1번만 사용하는 경우
                ...
                if (cRcp.abUsePointY_Two[i])
                {
                  ...
                  sZeroData[iPointIndex2].ProbeType = enumProbe.PR3;
                }
              }
              else if (cSys.bUsePB3 == false && cSys.bUsePB4 && cRcp.abUsePointX[i])
              {
                ...
                sZeroData[iPointIndex1].X = i;
                sZeroData[iPointIndex1].PointType = enumPoint.Y1;
                sZeroData[iPointIndex1].ProbeType = enumProbe.PR4;
            ...
            try
            {
              string strFileFullPath = CVo.LOG_PATH_TEST + CVo.LOG_NAME_TEST_ZEROSET2;
              if (System.IO.Directory.Exists(CVo.LOG_PATH_TEST) == false)
              {
                System.IO.Directory.CreateDirectory(CVo.LOG_PATH_TEST);
              }
